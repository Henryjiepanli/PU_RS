...
class dynamic_dual_uncertainty_aware_con_unit(nn.Module):
    ...
    def forward(self, low, high, predict):
        # 先计算高分辨率的前景特征，供低分辨率参考
        high_fore, high_back, contrastive_loss_high = self.UACL_2(high, predict)

        # 使用 high_fore 作为正样本参考，引导 low 的对比学习
        low_fore, low_back, contrastive_loss_low = self.UACL_1(
            low,
            F.interpolate(predict, size=low.shape[2:], mode='bilinear', align_corners=True))
...
class DynamicDualUncertaintyAwareContrastiveLearning(nn.Module):
    def __init__(self, feature_dim, sigma=0.1, learnable_sigma=False, centers=None):
        ...
        self.centers = nn.Parameter(torch.randn(5)) if centers is None else centers
    ...
    def forward(self, feature_map, pred_map):
        prob_map = self.prob(pred_map)
        fore_uncertainty_map = torch.clamp(prob_map - 0.5, min=0)
        back_uncertainty_map = torch.clamp(0.5 - prob_map, min=0)

        fore_rank_map = self.soft_rank_gaussian(fore_uncertainty_map)
        back_rank_map = self.soft_rank_gaussian(back_uncertainty_map)

        fore_weighted_features = feature_map * fore_rank_map.sum(dim=-1, keepdim=True).squeeze(dim=-1)
        back_weighted_features = feature_map * back_rank_map.sum(dim=-1, keepdim=True).squeeze(dim=-1)
