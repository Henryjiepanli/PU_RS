...
class MassBuildDataset(Dataset):
    def __init__(self, data_root='/data/henry/Code/Building_ISPRS/data/mass_build', mode='train', img_dir='train_images', mask_dir='train_masks',
    ...
    def __getitem__(self, index):
        ...
        # print(self.mode, np.unique(mask))
    ...
    def load_img_and_mask(self, index):
        ...
        img = np.array(Image.open(img_name).convert('RGB')).astype(np.uint8)
        mask = np.array(Image.open(mask_name).convert('L')).astype(np.float32)
        return img, mask
